...
const depression = new Scene('depression',
    ...
    (ctx) => {
        ...
        counter += determineDepressionResponse(ctx.message.text);
    },
    (ctx) => {
        ctx.scene.next();
        ctx.reply('Вопрос №3:' + '\n' + 'У меня бывают периоды плача или близости к слезам.', null, DEPRESSION_BUTTONS);
        counter += reverseScore(determineDepressionResponse(ctx.message.text));
    },
    (ctx) => {
        ctx.scene.next();
    ...
    (ctx) => {
        ...
        updateResult(ctx.message.from_id, 'depression', counter, sanity);
        counter = 0;
    }
);
