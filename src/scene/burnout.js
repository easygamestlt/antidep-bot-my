...
const burnout = new Scene('burnout',
    ...
    (ctx) => {
        ...
        exhaustion += determineBurnoutResponse(ctx.message.text);
    },
    (ctx) => {
        ctx.scene.next();
    ...
    (ctx) => {
        ...
        reduction += determineBurnoutResponse(ctx.message.text);
    },
    (ctx) => {
        ctx.scene.next();
        ctx.reply('Вопрос №6:' + '\n' + 'После работы на некоторое время хочется уединиться от всех и всего.', null, BURNOUT_BUTTONS
        );
        depersonalization += determineBurnoutResponse(ctx.message.text);
    },
    (ctx) => {
        ctx.scene.next();
    ...
    (ctx) => {
        ...
        updateBurnout(ctx.message.from_id, exhaustion, reduction, depersonalization, total_burnout);
        exhaustion = 0;
        depersonalization = 0;
        reduction = 0;
    }
